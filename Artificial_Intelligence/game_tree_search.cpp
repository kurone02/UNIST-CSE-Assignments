...
int GameTree::testTerminate(const bstate &node) {
    ...
    bool can_O_win = false, can_X_win = false;
    for(auto win_code: ws_code) {
        bcode o = (O_pos & win_code);
        bcode x = (X_pos & win_code);
        can_O_win |= (x == 0);
        can_X_win |= (o == 0);
        if(o == win_code) return O;
        if(x == win_code) return X;
    }
    
    if((O_pos | X_pos) == spec.getTerminalMask()) return DRAW;
    if(!can_O_win && !can_X_win) return EARLY_DRAW;
    return ONGOING;
}
...
int GameTree::heuristic(const bstate &state) {
    ...
    // (the_number_of_winning_seq_for_O + 100 * the_number_of_winning_seq_that_X_hasnot_won) - (the_number_of_winning_seq_for_O +  + 100 * the_number_of_winning_seq_that_O_hasnot_won)
...
void GameTree::printNode(const GameSpec &spec, const VisitNode &node) {
    ...
    if(node.state->isPruned) {
        ...
        switch(testTerminate(node.state->state)) {
            case DRAW: case EARLY_DRAW:
                std::cout << "DRAW payoff=0\n";
                break;
            case O:
                std::cout << "DRAW payoff=1\n";
                break;
            case X:
                std::cout << "DRAW payoff=-1\n";
                break;
...
int GameTree::select_move(const bstate &node, int alpha, int beta, const int &current_player) {
    std::vector<bstate> childNodes = generateMoves(node, current_player);
    int value = -INF, move_id = -1;
    bcode pos = (current_player == 1)? getLowerBitCode(node) : getUpperBitCode(node);
    for(bstate child: childNodes) {
        int new_val = -negamax(child, -beta, -alpha, -current_player);
        if(value < new_val) {
            bcode child_pos = (current_player == 1)? getLowerBitCode(child) : getUpperBitCode(child);
            move_id = __builtin_ffs(pos ^ child_pos) - 1;
            value = new_val;
        }
        alpha = std::max(alpha, value);
        if(alpha >= beta) {
...
void GameTree::playAsFirstPlayer(const GameSpec& spec) {
    this->spec = CompactGameSpec(spec);
    bstate current_state = 0;
    std::cin.tie(0);
    while(true) {
        // Check terminal node
        int is_terminal = testTerminate(current_state);
        if(is_terminal != ONGOING && is_terminal != EARLY_DRAW) break;

        // My turn
        int move_id = select_move(current_state, -INF, INF, 1);
        std::cout << this->spec.convertFromCompactToOriginalMoveId(move_id) << std::endl;
        current_state |= 1ll << move_id;

        // std::cerr << "TERMINAL1: " << current_state << ' ' << testTerminate(current_state) << std::endl;

        // opponent's turn
        std::cin >> move_id;
        current_state |= 1ll << (this->spec.convertFromOriginalToCompactMoveId(move_id) + INT_BIT_NUM);
        // std::cerr << "TERMINAL2: " << current_state << ' ' << testTerminate(current_state) << std::endl;
    }
    // std::cerr << "DUMA: " << ' ' << testTerminate(current_state) << std::endl;
}
...
void GameTree::playAsSecondPlayer(const GameSpec& spec) {
    ...
    while(true) {
        ...
        int move_id = 0;
        std::cin >> move_id;
        current_state |= 1ll << this->spec.convertFromOriginalToCompactMoveId(move_id);

        // My turn
        move_id = select_move(current_state, -INF, INF, -1);
        std::cout << this->spec.convertFromCompactToOriginalMoveId(move_id) << std::endl;
        current_state |= 1ll << (move_id + INT_BIT_NUM);
    }
}
