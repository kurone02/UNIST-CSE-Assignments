...
class Tree_t {
    ...
    protected:
        ...
        // Additional methods to make our life easier
        void SetL(Node_t<keyT, valT> *parent, Node_t<keyT, valT> *child){
            if(child != NULL) child->parent = parent;
            if(parent != NULL) parent->left = child;
        }

        void SetR(Node_t<keyT, valT> *parent, Node_t<keyT, valT> *child){
            if(child != NULL) child->parent = parent;
            if(parent != NULL) parent->right = child;
        }

        void SetLink(Node_t<keyT, valT> *parent, Node_t<keyT, valT> *child, bool inRight){
    ...
    public:
        ...
        void rotate(Node_t<keyT, valT>* n, bool direction) {
            Node_t<keyT, valT> *p = n->parent;
            Node_t<keyT, valT> *child;
            if(direction){
                child = n->left;
            ...
            if(p != NULL) SetLink(p, child, p->right == n);
            else if(child != NULL) child->parent = NULL;

            if(child->parent == NULL) root = child;
        }

        string to_string_in_order(Node_t<keyT, valT>* root) {
            stringstream ret;
            if(root) {
                if(root->left) ret << to_string_in_order(root->left);
                ret << root->key << ' ' << root->value << '\n';
                if(root->right) ret << to_string_in_order(root->right);
            }
            return ret.str();
        }

        string to_string_in_order(void) {
            return to_string_in_order(root);
        }
};
#endif
